...
rec {
  ...
  getDir = dir:
    if pathExists dir && pathIsDirectory dir then
      mapAttrs isFile (readDir dir)
    else
      [ ];
  ...
  findNixFiles = path:
    let
      ...
      allFiles = if isDir then dirFiles path else [ path ];
      nixFiles = filter isNix allFiles;
      formatter = if isDir then file: "${path}/${file}" else file: path;
