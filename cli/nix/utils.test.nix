    ...
    "getDir" =
        {
            ...
            "non-empty list for existing path" =
                length (attrNames (utils.getDir ./.)) != 0;
        };

    "dirFiles" =
        {
            "empty list for non-existent path" =
                utils.dirFiles "/aosfijweeo" == [];
            "non-empty list for existing path" =
                length (utils.dirFiles ./.) != 0;
        };

    "isNix" =
        {
            "true for nix files" =
                utils.isNix "./nixpkgs.nix" == true;
            "false for non-nix files" =
                utils.isNix "./utils.foo" == false;
        };
    ...
    "isTestSuite" =
        ...
        in {
            "true for test suites" =
                utils.isTestSuite realTest == true;
            "false for non-test suites" =
                utils.isTestSuite nonTest == false;
        };
}